...
inline void fillQvals(const SpecularElementIter it_begin, const SpecularElementIter it_end,
...
inline void extractSldThicknessRoughnessAndQvalsFromSample(const MultiLayer& multi_layer,
                                                           ...
                                                           std::vector<double_t>& roughnesses)
{
    ...
    const std::unique_ptr<MultiLayer> avr_sample =
        IComputationUtils::CreateAveragedMultilayer(multi_layer, sim_options);
...
inline void heavyComputation(std::vector<double_t>& reflectometry,
                             ...
                             const std::vector<double_t>& roughnesses,
                             const std::vector<double_t> qvals)
{
...
void SpecularComputation::runProtected()
{
    ...
    std::vector<double_t> thicknesses;
    std::vector<double_t> roughnesses;
    extractSldThicknessRoughnessAndQvalsFromSample(*mP_multi_layer, m_sim_options, slds,
                                                   thicknesses, roughnesses);

    std::vector<double_t> qvals;
    fillQvals(m_begin_it, m_end_it, qvals);

    size_t npoints = qvals.size();
    std::vector<double> reflectometry(npoints);
    heavyComputation(reflectometry, slds, thicknesses, roughnesses, qvals);
